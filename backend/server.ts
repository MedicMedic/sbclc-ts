...
// -----------------------------
// Interfaces (selected, important ones)
// -----------------------------
interface DBUser {
  user_id: number;
  email: string;
...
interface Quotation {
  ...
  contact_position?: string | null;
  consignee_position?: string | null;
  address?: string | null;
...
function buildInsert(table: string, payload: Record<string, any>) {
...
const requireRole = (allowed: string[] | string) => {
...
app.get("/api/users", verifyToken, requireRole(["admin"]), (req, res) => {
  try {
    const rows = db
      .prepare(
        `SELECT user_id, email, full_name, first_name, last_name, avatar, role, department, is_active, created_at, updated_at, last_login
...
app.get("/api/users/:id", verifyToken, (req, res) => {
  try {
    const row = db
      ...
      .get(req.params.id) as DBUser | undefined;
...
app.put("/api/profile", verifyToken, upload.single("avatar"), (req, res) => {
  try {
    ...
    if (req.body.new_password) {
      ...
      if (
        !bcrypt.compareSync(req.body.current_password, existing.password_hash)
      ) {
        if (req.file) {
          try {
            fs.unlinkSync(path.join(uploadDir, req.file.filename));
      ...
      payload.password_hash = bcrypt.hashSync(req.body.new_password, 10);
      requiresReauth = true;
    }
    ...
    const updatedUser = db
      .prepare(
        `SELECT user_id, email, first_name, last_name, full_name, role, department, avatar
    ...
    if (req.file) {
      try {
        fs.unlinkSync(
          path.join(__dirname, "uploads/avatars", req.file.filename)
        );
      } catch {}
    }
  ...
  try {
    ...
    if (req.body.modules && typeof req.body.modules === 'object') {
      const insertPerm = db.prepare(
        "INSERT INTO role_permissions (role_code, module_id, action, is_granted) VALUES (?, ?, ?, 1)"
      );
...
app.put("/api/roles/:id", verifyToken, requireRole(["admin"]), (req, res) => {
  try {
    ...
    if (!upd) return res.status(400).json({ message: "No fields to update" });
    db.prepare(upd.sql).run(...upd.values);
  ...
  (req, res) => {
    try {
      ...
      const role = db
        ...
        .get(id) as { role_code: string } | undefined;
      ...
      const usersWithRole = db
        ...
        .get(role.role_code) as { count: number };
...
app.get("/api/roles/:roleCode/permissions", verifyToken, (req, res) => {
  try {
    ...
    const role = db
      .prepare(
        "SELECT role_code FROM roles WHERE role_code = ? AND is_active = 1"
      )
      .get(roleCode) as Role | undefined;
    if (!role) return res.status(404).json({ message: "Role not found" });

    const rows = db
      .prepare(
        "SELECT module_id, action, is_granted FROM role_permissions WHERE role_code = ? AND is_granted = 1"
      )
      .all(roleCode) as { module_id: string; action: string }[];
        ...
        if (Array.isArray(permissions)) {
          ...
          for (const [module_id, actions] of Object.entries(permissions)) {
            (actions as string[]).forEach((a) =>
              insertStmt.run(roleCode, module_id, a)
            );
          }
        }
      })();
...
app.get(
  ...
  (req, res) => {
    try {
      const rows = db
        .prepare(
          "SELECT DISTINCT module_id, action FROM role_permissions ORDER BY module_id, action"
        )
        .all();
      const grouped: Record<string, string[]> = {};
      (rows as { module_id: string; action: string }[]).forEach((r) => {
        if (!grouped[r.module_id]) grouped[r.module_id] = [];
...
app.get("/api/clients/:id", verifyToken, (req, res) => {
  try {
    const client = db
      ...
      .get(req.params.id) as Client | undefined;
...
app.delete("/api/clients/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE clients SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE client_id = ?"
    ).run(req.params.id);
...
app.get("/api/service-types", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM service_types WHERE is_active = 1 ORDER BY service_type_name"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/service-types", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO service_types (service_type_name, category, description, is_active) VALUES (?, ?, ?, ?)"
    ).run(
      req.body.service_type_name,
      req.body.category,
      req.body.description || null,
      req.body.is_active !== undefined ? (req.body.is_active ? 1 : 0) : 1
    );
...
app.delete("/api/service-types/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE service_types SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE service_type_id = ?"
    ).run(req.params.id);
...
app.post("/api/categories", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO categories (category_name, category_type, parent_category_id, description, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(
...
app.delete("/api/categories/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE categories SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE category_id = ?"
    ).run(req.params.id);
...
app.post("/api/ports", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO ports (port_name, port_code, country, port_type, is_active) VALUES (?, ?, ?, ?, ?)"
    ).run(
      req.body.port_name,
      req.body.port_code,
      req.body.country || null,
      req.body.port_type || "seaport",
      req.body.is_active !== undefined ? (req.body.is_active ? 1 : 0) : 1
    );
...
app.delete("/api/ports/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE ports SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE port_id = ?"
    ).run(req.params.id);
...
app.get("/api/shipping-lines", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM shipping_lines WHERE is_active = 1 ORDER BY line_name"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/shipping-lines", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO shipping_lines (line_name, line_code, country, contact_email, contact_phone, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(
      req.body.line_name,
      req.body.line_code,
      req.body.country || null,
...
app.delete("/api/shipping-lines/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE shipping_lines SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE shipping_line_id = ?"
    ).run(req.params.id);
...
app.get("/api/currencies", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM currencies WHERE is_active = 1 ORDER BY currency_code"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.delete("/api/currencies/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE currencies SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE currency_id = ?"
    ).run(req.params.id);
...
app.get("/api/truckers", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM truckers WHERE is_active = 1 ORDER BY trucker_name"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/truckers", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO truckers (trucker_name, trucker_type, contact_person, contact_phone, address, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(
      req.body.trucker_name,
      req.body.trucker_type,
...
app.delete("/api/truckers/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE truckers SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE trucker_id = ?"
    ).run(req.params.id);
...
app.get("/api/warehouses", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM warehouses WHERE is_active = 1 ORDER BY warehouse_name"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/warehouses", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO warehouses (warehouse_name, warehouse_code, address, warehouse_type, capacity, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(
      req.body.warehouse_name,
      req.body.warehouse_code,
      req.body.address,
      req.body.warehouse_type || "standard",
      req.body.capacity || 0,
      req.body.is_active !== undefined ? (req.body.is_active ? 1 : 0) : 1
    );
...
app.delete("/api/warehouses/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE warehouses SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE warehouse_id = ?"
    ).run(req.params.id);
...
app.get("/api/container-sizes", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM container_sizes WHERE is_active = 1 ORDER BY display_order"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/container-sizes", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO container_sizes (size_name, size_code, description, display_order, is_active) VALUES (?, ?, ?, ?, ?)"
    ).run(
      req.body.size_name,
      req.body.size_code || null,
      req.body.description || null,
      req.body.display_order || 0,
      req.body.is_active !== undefined ? (req.body.is_active ? 1 : 0) : 1
    );
...
app.delete("/api/container-sizes/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE container_sizes SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE container_size_id = ?"
    ).run(req.params.id);
...
app.get("/api/truck-sizes", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM truck_sizes WHERE is_active = 1 ORDER BY display_order"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/truck-sizes", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO truck_sizes (size_name, size_code, truck_type, description, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(
      req.body.size_name,
      req.body.size_code || null,
      req.body.truck_type || "other",
      req.body.description || null,
      req.body.display_order || 0,
      req.body.is_active !== undefined ? (req.body.is_active ? 1 : 0) : 1
    );
...
app.delete("/api/truck-sizes/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE truck_sizes SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE truck_size_id = ?"
    ).run(req.params.id);
...
app.get("/api/service-rates", verifyToken, (req, res) => {
  try {
    const rows = db
      .prepare(
        "SELECT * FROM service_rates WHERE is_active = 1 ORDER BY service_type, category"
      )
      .all();
    res.json(rows);
  } catch (err) {
...
app.post("/api/service-rates", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO service_rates (service_type, category, unit_price, description, is_active) VALUES (?, ?, ?, ?, ?)"
    ).run(
...
app.delete("/api/service-rates/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE service_rates SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE rate_id = ?"
    ).run(req.params.id);
...
app.post("/api/expense-categories", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO expense_categories (category_name, category, description, is_active) VALUES (?, ?, ?, ?)"
    ).run(
      req.body.category_name,
      req.body.category,
      req.body.description || null,
...
app.delete("/api/expense-categories/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE expense_categories SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE category_id = ?"
    ).run(req.params.id);
...
app.post("/api/statuses", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO statuses (status_code, status_name, category, color, sequence, description, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)"
    ).run(
...
app.delete("/api/statuses/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE statuses SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE status_id = ?"
    ).run(req.params.id);
...
app.post("/api/milestones", verifyToken, (req, res) => {
  try {
    ...
    if (missing)
      return res.status(400).json({ message: `${missing} is required` });
    db.prepare(
      "INSERT INTO milestones (milestone_code, milestone_name, service_type, sequence_order, description, estimated_days, notify_before_days, is_required, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ).run(
      req.body.milestone_code,
      req.body.milestone_name,
      req.body.service_type,
      req.body.sequence_order,
      req.body.description || null,
      req.body.estimated_days || 0,
      req.body.notify_before_days || 0,
      req.body.is_required !== undefined ? (req.body.is_required ? 1 : 0) : 1,
      req.body.is_active !== undefined ? (req.body.is_active ? 1 : 0) : 1
    );
...
app.delete("/api/milestones/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE milestones SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE milestone_id = ?"
    ).run(req.params.id);
...
app.get("/api/quotations/:id", verifyToken, (req, res) => {
  try {
    const q = db
      ...
      .get(req.params.id) as Quotation | undefined;
    if (!q) return res.status(404).json({ message: "Quotation not found" });
    const items = db
      .prepare(
        "SELECT * FROM quotation_items WHERE quotation_id = ? ORDER BY item_sequence"
      )
      .all(req.params.id) as QuotationItem[];
    res.json({ ...q, items });
  } catch (err) {
...
app.post("/api/quotations", verifyToken, (req, res) => {
  try {
    ...
    const payload: any = {
      ...
      is_deleted:
        req.body.is_deleted !== undefined ? (req.body.is_deleted ? 1 : 0) : 0,
    };

    const { sql, values } = buildInsert("quotations", payload);
    const result = db.prepare(sql).run(...values);
    const quotation_id = result.lastInsertRowid as number;

    if (Array.isArray(req.body.items) && req.body.items.length > 0) {
      const insertItem = db.prepare(
        `INSERT INTO quotation_items (quotation_id, item_sequence, description, category, warehouse, container_size, equipment_type, currency, quantity, unit, rate, amount, custom_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      );
      db.transaction(() => {
        for (let i = 0; i < req.body.items.length; i++) {
          ...
          insertItem.run(
            quotation_id,
            it.item_sequence || i + 1, // Use provided sequence or auto-increment
            it.description,
            it.category,
            it.warehouse || null,
            it.container_size || it.containerSize || null,
            ...
            it.rate || 0,
            it.amount || 0,
            it.custom_data ? JSON.stringify(it.custom_data) : null
          );
        }
      })();
    }
...
app.delete("/api/quotations/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE quotations SET is_deleted = 1, updated_at = CURRENT_TIMESTAMP WHERE quotation_id = ?"
    ).run(req.params.id);
...
app.delete("/api/quotation-items/:id", verifyToken, (req, res) => {
  try {
    db.prepare("DELETE FROM quotation_items WHERE item_id = ?").run(
      req.params.id
    );
...
app.get("/api/quotations/stats/summary", verifyToken, (req, res) => {
  try {
    ...
    const pendingRow = db
      .prepare(
        "SELECT COUNT(*) as count FROM quotations WHERE status = 'pending_approval' AND is_deleted = 0"
      )
      .get() as { count: number };
    
    const approvedRow = db
      .prepare(
        "SELECT COUNT(*) as count FROM quotations WHERE status = 'approved' AND is_deleted = 0"
      )
      .get() as { count: number };
    
    const clientReviewRow = db
      .prepare(
        "SELECT COUNT(*) as count FROM quotations WHERE status = 'client_review' AND is_deleted = 0"
      )
      .get() as { count: number };
    ...
    const approvedQuotations = db
      .prepare(
        "SELECT total_amount, base_currency, exchange_rate FROM quotations WHERE status = 'approved' AND is_deleted = 0"
      )
      .all() as Array<{ total_amount: number; base_currency: string; exchange_rate: number }>;
    ...
    approvedQuotations.forEach(q => {
      ...
      if (currency === "PHP") {
        ...
        const currencyData = db
          ...
          .get(currency) as { exchange_rate: number } | undefined;
...
app.put("/api/quotations/:id/revive", verifyToken, (req, res) => {
  try {
    ...
    db.prepare(
      "UPDATE quotations SET is_deleted = 0, updated_at = CURRENT_TIMESTAMP WHERE quotation_id = ?"
    ).run(id);
...
app.put("/api/quotations/:id/approve", verifyToken, (req, res) => {
  try {
    ...
    db.transaction(() => {
      db.prepare(
        "UPDATE quotations SET status = 'approved', approved_by = ?, updated_at = CURRENT_TIMESTAMP WHERE quotation_id = ?"
      ).run(approvedBy, id);

      // Log to approval history if needed
      const user = db
        .prepare("SELECT user_id FROM users WHERE full_name = ? OR email = ?")
        .get(approvedBy, approvedBy) as { user_id: number } | undefined;

      if (user) {
        db.prepare(
          `INSERT INTO approval_history (transaction_type, transaction_id, reference_no, action, action_by, action_by_name, action_date, comments)
...
app.put("/api/quotations/:id/reject", verifyToken, (req, res) => {
  try {
    ...
    db.transaction(() => {
      db.prepare(
        "UPDATE quotations SET status = 'rejected', updated_at = CURRENT_TIMESTAMP WHERE quotation_id = ?"
      ).run(id);
...
app.delete("/api/quotations/:id/permanent", verifyToken, requireRole(["admin"]), (req, res) => {
  try {
    ...
    db.transaction(() => {
      ...
      db.prepare(
        "DELETE FROM approval_history WHERE transaction_type = 'quotation' AND transaction_id = ?"
      ).run(id);
    })();
...
app.get("/api/approvals/stats", verifyToken, (req, res) => {
  try {
    ...
    const rejectedRow = db
      .prepare(
        "SELECT COUNT(*) as count FROM quotations WHERE status = 'rejected' AND is_deleted = 0"
      )
      .get() as { count: number };
...
app.get("/api/approvals/quotation/:id", verifyToken, (req, res) => {
  try {
    ...
    return sendError(
      res,
      err,
      "Failed to fetch quotation details for approval"
    );
  }
});
...
app.post("/api/approvals/quotation/:id/approve", verifyToken, (req, res) => {
  try {
    ...
    const user = db
      ...
      .get(req.user!.id) as { full_name?: string } | undefined;
    ...
    db.transaction(() => {
      ...
      db.prepare(
        "UPDATE quotations SET status = ?, approved_by = ?, updated_at = CURRENT_TIMESTAMP WHERE quotation_id = ?"
      ).run("approved", actorName, id);
...
app.post("/api/approvals/quotation/:id/reject", verifyToken, (req, res) => {
  try {
    ...
    db.transaction(() => {
      db.prepare(
        "UPDATE quotations SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE quotation_id = ?"
      ).run("rejected", id);
...
app.get("/api/approvals/quotation/:id/history", verifyToken, (req, res) => {
  try {
    ...
    const rows = db
      .prepare(
        "SELECT * FROM approval_history WHERE transaction_type = 'quotation' AND transaction_id = ? ORDER BY action_date DESC"
      )
      .all(id) as ApprovalHistoryRow[];
...
app.delete("/api/bookings/:id", verifyToken, (req, res) => {
  try {
    db.prepare(
      "UPDATE bookings SET is_deleted = 1, updated_at = CURRENT_TIMESTAMP WHERE booking_id = ?"
    ).run(req.params.id);
...
app.get("/api/approval_matrix", verifyToken, (req, res) => {
  try {
    ...
    const levelStmt = db.prepare(
      "SELECT * FROM approval_levels WHERE rule_id = ? ORDER BY level ASC"
    );

    const fullRules = rules.map((rule: any) => ({
      ...rule,
